		...
		<>
			...
			<div className="main-content-fakultas">
				<div className="container-fakultas">
					...
					<p className="header-text-fakultas">Simak fakultas yang sesuai dengan passion kalian</p>
				</div>
					...
					{data.map((item, index) => (
						<div key={index} className='fakultas-list'>
							...
							<div className='header-and-link'>
								<h2 className='the-header'>{item.fakultas_name}</h2>
								<Link to={item.to} className='the-link'>Lihat Selengkapnya</Link>
							</div>
						</div>
					))}
				</div>
